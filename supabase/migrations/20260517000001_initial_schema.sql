...
comment on table public.users is 'アプリユーザー（Supabase Auth と 1:1）';
comment on column public.users.daily_ai_used is '当日のAI生成利用回数';
comment on column public.users.daily_ai_reset_at is 'daily_ai_used をリセットした日付';
...
comment on table public.artists is 'ユーザーが登録した推し';
...
comment on table public.venues is '全国主要ライブ会場マスタ（SSG用）';
...
comment on table public.plans is 'AI生成された遠征プラン';
comment on column public.plans.user_id is 'nullable: 未ログインゲストのプランは NULL';
comment on column public.plans.plan_json is '行程・宿泊・交通・アフィリエイトURL等を格納する JSONB';
comment on column public.plans.share_token is '共有用ランダムトークン（32文字 hex）';
...
comment on table public.affiliate_clicks is 'アフィリエイトリンクのクリック計測';
...
comment on table public.plan_records is '遠征後の実績記録（v1.2）';
